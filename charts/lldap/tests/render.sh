...
grep -Fq "'charts/lldap/**'" "$workflow"
grep -Fq "'environments/dev/lldap/**'" "$workflow"
grep -Eq '^          - lldap$' "$workflow"
lldap_sync_step="$(sed -n '/- name: Sync LLDAP credentials/,/- name: Sync Authelia credentials/p' "$workflow")"
grep -Fq 'LLDAP_JWT_SECRET: ${{ secrets.LLDAP_JWT_SECRET }}' <<<"$lldap_sync_step"
grep -Fq 'LLDAP_KEY_SEED: ${{ secrets.LLDAP_KEY_SEED }}' <<<"$lldap_sync_step"
grep -Fq 'LLDAP_ADMIN_PASSWORD: ${{ secrets.LLDAP_ADMIN_PASSWORD }}' <<<"$lldap_sync_step"
grep -Fq 'LLDAP_AUTHELIA_PASSWORD: ${{ secrets.LLDAP_AUTHELIA_PASSWORD }}' <<<"$lldap_sync_step"
grep -Fq 'POSTGRES_PASSWORD: ${{ secrets.POSTGRES_PASSWORD }}' <<<"$lldap_sync_step"
grep -Fq "'\$value|@uri'" <<<"$lldap_sync_step"
grep -Fq 'kubectl -n infra create secret generic lldap-secrets' <<<"$lldap_sync_step"
grep -Fq -- '--from-file=lldap-jwt-secret=' <<<"$lldap_sync_step"
grep -Fq -- '--from-file=lldap-key-seed=' <<<"$lldap_sync_step"
grep -Fq -- '--from-file=lldap-ldap-user-name=' <<<"$lldap_sync_step"
grep -Fq -- '--from-file=lldap-ldap-user-pass=' <<<"$lldap_sync_step"
grep -Fq -- '--from-file=base-dn=' <<<"$lldap_sync_step"
grep -Fq -- '--from-file=database-url=' <<<"$lldap_sync_step"
grep -Fq -- '--from-file=authelia-password=' <<<"$lldap_sync_step"
grep -Fq 'kubectl -n infra rollout restart deployment/lldap' <<<"$lldap_sync_step"
grep -Fq 'kubectl -n infra rollout status deployment/lldap' <<<"$lldap_sync_step"
