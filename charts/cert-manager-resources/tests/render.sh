...
chart_dir="$repo_root/charts/cert-manager-resources"
dev_values="$repo_root/environments/dev/cert-manager-resources/values.yaml"
rendered="$(mktemp)"
trap 'rm -f "$rendered"' EXIT

helm template cert-manager-resources "$chart_dir" \
...
echo 'cert-manager resources render checks passed'
